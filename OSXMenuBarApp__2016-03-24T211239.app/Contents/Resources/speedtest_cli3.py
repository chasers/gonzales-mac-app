...
def publishSpeeds(apikey,dlspeed,ulspeed,mac,ip,isp):
    f = {'api_key': apikey, 'mac': hex(mac), 'dl': dlspeed, 'ul': ulspeed, 'ip': ip, 'isp': isp}
    q = urllib.urlencode(f)
    h = HTTPSConnection("script.google.com")
    url = '/macros/s/AKfycbzFtkBXRdn9zkSZY3yD-r5cxiT6GrsNbklNSMqpA5A4xTnnjR1i/exec?'
    h.request("GET",url+q)
    r = h.getresponse()
...
def speedtest():
    ...
    parser.add_argument("--apikey", type=str, required=True,
                        help='APIKEY to use when publishing speeds')
    ...
    if isinstance(options, tuple):
        ...
        args = options
    del options
    print 'apikey is %s' % args.apikey
    mac = get_mac()
    print 'mac is %s' % hex(mac)
    ...
    if not args.simple:
        print_('Publishing speeds...')
        junk = publishSpeeds(args.apikey,dlspeed,ulspeed,mac,config['client']['ip'],config['client']['isp'])
